...
import {TestDataService} from "../../../data/dao/impl/TestDataService";
...
export class MainComponent implements OnInit {
    ...
    // названия cookies
    readonly cookieTaskSeachValues = 'todoSearchValues'; // для сохранения параметров поиска в формате JSON
    readonly cookieShowStat = 'todoShowStat'; // показывать или нет статистику
    readonly cookieShowMenu = 'todoShowMenu'; // показывать или нет статистику
    readonly cookieShowSearch = 'todoShowSearch'; // показывать или нет инструменты поиска
    readonly cookieLang = 'todoLang'; // язык интерфейса
    ...
    constructor(
        ...
        private cookieService: CookieService // работа с куками браузера
    ...
    private initData(): void {
        ...
        this.bffService.requestUserProfile().subscribe(
            {
                // успешное выполнение
                next: ((response: User) => {
    ...
    toggleMenu(): void {
        ...
        this.cookieService.set(this.cookieShowMenu, String(this.menuOpened));
    }
    ...
    selectCategory(category: Category): void {
        ...
        // обновляем статистику - должно быть обязательно перед получением самих задач
        ...
        if (this.selectedCategory != null) { // обновляем статистику дешбоарда вверху страницы - данными категории
            ...
            this.statService.getOverallStat(this.user.email).subscribe((result => { // получить общую статистику
                    ...
                    // (не совсем верный подход, но рабочий) - если статистики нет - значит это новый пользователь (т.к. в таблице stat создается 1 строка только 1 раз и потом только обновляется),
                    // поэтому создаем тестовые данные и заново обновляем список задач, категорий, статистику и пр
                    // получается при первом обращении к приложению - пользователю будут 1 раз созданы тестовые данные
                    if (this.stat == null) {

                        this.testDataService.createTestData(this.user.email).subscribe(result => {
                                // в result запишется объект pageable из backend
                                this.initData(); // заново загрузить все данные (т.к. только что были добавлены тестовые данные)
                            }
                        );
    ...
    toggleStat(showStat: boolean): void {
        ...
        // сохраняем в cookies текущее значение
        this.cookieService.set(this.cookieShowStat, String(showStat));
    ...
    searchTasks(searchTaskValues: TaskSearchValues): void {
        ...
        // сохраняем в cookies текущее значение
        this.cookieService.set(this.cookieTaskSeachValues, JSON.stringify(this.taskSearchValues));
    ...
    toggleSearch(showSearch: boolean): void {
        ...
        // сохраняем в cookies текущее значение
        this.cookieService.set(this.cookieShowSearch, String(showSearch));
    ...
    initSearchCookie(): boolean {
        ...
        const cookie = this.cookieService.get(this.cookieTaskSeachValues);
    ...
    initLangCookie(): void {
        ...
        const val = this.cookieService.get(this.cookieLang);
    ...
    initShowMenuCookie(): void {
        const val = this.cookieService.get(this.cookieShowMenu);
        if (val) { // если кук найден
    ...
    initShowSearchCookie(): void {
        const val = this.cookieService.get(this.cookieShowSearch);
        if (val) { // если кук найден
    ...
    initShowStatCookie(): void {
        if (!this.isMobile) { // если моб. устройство, то не показывать статистику
            const val = this.cookieService.get(this.cookieShowStat);
            if (val) { // если кук найден
    ...
    settingsChanged(priorities: Priority[]): void { // priorities - измененный список приоритетов
        ...
        this.cookieService.set(this.cookieLang, this.translate.currentLang);
    ...
    private exchangeRefreshToken(): void {
        this.bffService.exchangeRefreshToken().subscribe(
            {
    ...
    logoutAction(): void {
        this.bffService.logout().subscribe({
